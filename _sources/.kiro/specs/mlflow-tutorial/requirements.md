...
**Objective:** As a MLエンジニア・データサイエンティスト, I want OptunaによるハイパーパラメータPR最適化とMLflow Trackingを組み合わせた実験管理ワークフローを体験できること, so that 体系的なハイパーパラメータ探索とMLflowのExperiment比較機能の実用的な価値を理解できる

#### Acceptance Criteria
1. The Tutorial shall Optunaの`study.optimize()`内でMLflow runを自動作成し、各trialのパラメータ・メトリクスをMLflow Trackingに記録するノートブックを提供する
...
4. The Tutorial shall Optunaの`study.best_trial`からベストパラメータを取得し、そのrun IDからMLflow上のモデルをModel Registryに登録する手順を含める
5. The Tutorial shall Optunaのsamplerとして`TPESampler`（デフォルト）を使用し、外部インターネット接続不要で動作することを確認する
6. The Tutorial shall Optunaによる最適化のトライアル数・探索空間の設定（`trial.suggest_float` / `trial.suggest_int` / `trial.suggest_categorical`）のコード例を含める
